    ...
    <div className="p-4 lg:p-8 space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          >
            <DollarSign size={24} style={{ color: brandColor }} />
          </div>
          <div>
            <h1 className="text-3xl font-black text-white italic uppercase tracking-tighter mb-2">
              {lang === 'es' ? 'CIERRES' : 'CLOSED'} <span style={{ color: brandColor }}>{lang === 'es' ? 'COMERCIALES' : 'DEALS'}</span>
            </h1>
            <p className="text-zinc-500 font-bold italic uppercase tracking-widest">
              {lang === 'es' ? 'HISTORIAL DE MOVIMIENTOS Y COMISIONES' : 'TRANSACTION AND COMMISSION HISTORY'}
            </p>
          </div>
        </div>
