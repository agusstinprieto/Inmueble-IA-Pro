...
import { Camera, FileText, Mic, FileSignature, TrendingUp, AlertTriangle, CheckCircle, ArrowUpCircle, Activity } from 'lucide-react';
...
export default function UsageDashboardView({ lang, brandColor }: UsageDashboardViewProps) {
    const [stats, setStats] = useState<UsageStats | null>(null);
    const [loading, setLoading] = useState(true);
    const agencyId = ''; // TODO: Get from props or context
    ...
    return (
        <div className="p-4 lg:p-6 space-y-6">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-3xl font-black text-white italic uppercase tracking-tighter mb-2">
                            USO DE <span style={{ color: brandColor }}>RECURSOS</span>
                        </h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {(Object.entries(stats.features) as [string, { current: number; limit: number; percentage: number }][]).map(([feature, data]) => {
            ...
            {/* Upgrade Suggestion */}
            {(Object.values(stats.features) as { percentage: number; limit: number }[]).some(f => f.percentage >= 80 && f.limit !== -1) && (
